...
Run once. The venv lives in `.venv-scholar/` (gitignored) and persists across
refreshes. Redo only on a new machine, after deleting the venv, or if
`scripts/requirements-scholar.txt` changes.
