...
**Weekly refresh (~10–20 min for 39 papers):**
```bash
...
If the run prints `[BLOCKED]`, wait 24 h or switch networks; partial progress
is preserved (the file is rewritten atomically after every paper).

**Rollback:** `git rm _data/scholar_counts.yml` — the site falls back to
OpenAlex for every paper, exactly as it did before this pipeline shipped.
