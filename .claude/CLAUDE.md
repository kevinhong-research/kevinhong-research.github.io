...
| `_data/scholar_counts.yml` | **Auto-generated.** Google Scholar citation counts per DOI — see [`scripts/README.md`](../scripts/README.md) |
...
All repo maintenance scripts live in `scripts/`. The full index, the
"I want to… → run…" decision guide, and the deep pipeline docs (ASCII diagram,
cheat-sheet, gotchas, rollback) live in **[`scripts/README.md`](../scripts/README.md)** —
kept out of this always-loaded file to save context.

Key entry points:

- `./scripts/refresh_scholar.sh` — weekly refresh: promote forthcoming papers
  (Crossref) **then** refresh Google Scholar citation counts. Phase 2 tries the
  requests path and **auto-falls back to the browser fetcher** the moment Scholar
  captcha-blocks it (the usual outcome — a Chrome window opens). One push.
- `./scripts/fetch_scholar_browser.sh` — the browser fetcher itself (signed-in
  Chrome). Auto-invoked by `refresh_scholar.sh` on a block; run standalone to
  skip the requests probe. **The working path** for Scholar counts right now.
- `./scripts/refresh_forthcoming.sh` — promote `forthcoming: true` papers now in
  print, standalone (pure stdlib, no venv).
- `python scripts/fix_pub_titles.py` — Chicago title-case for
  `_data/publications.yml` (also the `.githooks/pre-commit` hook).
- `python scripts/audit_colors.py` — flag hard-coded color literals in site-owned UI.

**Script convention:** `*.sh` = thin wrappers that **commit + push** when something
changed; `*.py` = pure workers/utilities with no git side-effects. Run all from the
project root. The Scholar scripts need `.venv-scholar/` (`./scripts/setup_scholar.sh`);
forthcoming, title-case, and color-audit are pure stdlib.
